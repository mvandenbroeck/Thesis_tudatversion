...
#include "Eigen/Core"
#include "conio.h" // Required to use sleep command for debugging purposes
#include "windows.h" // Required to use sleep command for debugging purposes
...
ProblemRecurrenceRelations::ProblemRecurrenceRelations(
        ...
        useCSI_( useCSI ),
        useImprovedMass_( useImprovedMass ),
        useSeparateSum_( useSeparateSum ),
        order_( order ),
...
//    std::cout << "acc_curr = \n" << acc_curr << std::endl;
    ...
    else if ( useCSI_ == true )
    {
        double currentThrust = spacecraftPointer_->getCurrentResultantThrustForce();
        U8( 0 ) = -currentThrust /
                ( constantsPointer_->standardGravity_
                  * spacecraftPointer_->getSpecificImpulse() ); // Current thrust force = constant acc * current mass
    }
...
//                                    * boost::math::factorial<double>( k ) ); // no use of  k because k=1 corresponds to second derivative already
        /// Equation added by Michael Van den Broeck
        U8( k ) = - dT_res( k ) / ( constantsPointer_->standardGravity_ * spacecraftPointer_->getSpecificImpulse()
                                    * boost::math::factorial<double>( k ) ); // no use of  k because k=1 corresponds to second derivative already
        ///
    ...
    // Declare computed next state
    Vector11d x;

    if ( useSeparateSum_ == false )
    {
        // Declare variables
        ...
        if ( useImprovedMass_ == false )
        {
            x_8 = X8.adjoint() * T_vector; // scalar
        }
        else if ( useImprovedMass_ == true )
        {
            // Using alternative time h = t / dt:
            // dh = h - h_0 = t/dt - t_0/dt = dt/dt = 1.
            // The product of X8.adjoint() with a vector of ones is the same as taking the sum of X8
            // Declare x_8 to contain a zero value
            x_8.setZero( 1, 1 );
            X8 = 400.0 * X8;
            X8( 0 ) = X8( 0 ) / 400.0;
            x_8( 0, 0 ) = X8.sum();
        ...
        x << x_1( 0 ), x_2( 0 ), x_3( 0 ), x_4( 0 ), x_5( 0 ),
                x_6( 0 ), x_7( 0 ), x_8( 0 ), x_9( 0 ), x_10( 0 ), x_11( 0 );

    }
    else if ( useSeparateSum_ == true )
    {
        Eigen::MatrixXd xProduct( 11, order_ + 1 );

        xProduct.row( 0 ) = X1.cwiseProduct( T_vector ); // order+1 columns
        xProduct.row( 1 ) = X2.cwiseProduct( T_vector );
        xProduct.row( 2 ) = X3.cwiseProduct( T_vector );
        xProduct.row( 3 ) = X4.cwiseProduct( T_vector );
        xProduct.row( 4 ) = X5.cwiseProduct( T_vector );
        xProduct.row( 5 ) = X6.cwiseProduct( T_vector );
        xProduct.row( 6 ) = X7.cwiseProduct( T_vector );
        xProduct.row( 7 ) = X8.cwiseProduct( T_vector );
//        //??
//        std::cout << "xProduct first row:\n" << xProduct.row( 7 ) << std::endl;
        xProduct.row( 8 ) = X9.cwiseProduct( T_vector );
        xProduct.row( 9 ) = X10.cwiseProduct( T_vector );
        xProduct.row( 10 ) = X11.cwiseProduct( T_vector );

        double positiveSum, negativeSum;
        int i, j;
        // Loop over all indices in the same row of xProduct and sum the positive and negative entries separately
        for ( i = 0; i < xProduct.rows(); i++ )
        {
            positiveSum = 0.0;
            negativeSum = 0.0;

            for ( j = 0; j < xProduct.cols(); j++ )
            {
//                //??
//                std::cout << "xProduct( " << i << ", " << j << " ) = " << xProduct( i, j ) << std::endl;

                if ( xProduct( i, j ) >= 0.0 )
                {
                    positiveSum = positiveSum + xProduct( i, j );
                }
                else if ( xProduct( i, j ) < 0.0 )
                {
                    negativeSum = negativeSum + xProduct( i, j );
                }
            }
            x( i ) = positiveSum + negativeSum;
//            //??
//            std::cout << "positiveSum = " << positiveSum << std::endl;
//            std::cout << "negativeSum = " << negativeSum << std::endl;
//            std::printf("x( %2i ) = %5.6f \n", i, x( i ));
        }
    }
...
//    Sleep(1.0e6);
    ...
    spacecraftPointer_->setCurrentState( x );
